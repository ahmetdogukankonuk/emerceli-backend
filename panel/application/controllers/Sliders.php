...
class Sliders extends MY_Controller {
    ...
    public function add_slider(){
        ...
        if($this->form_validation->run()){
            ...
            if(empty($_FILES[$file_field]["name"])){
                $this->setFlashAlert("error", "Please select an image");
                redirect(base_url("sliders/new"));
            }
    
            $file_name = convertToSEO(pathinfo($_FILES[$file_field]["name"], PATHINFO_FILENAME)) . "." . pathinfo($_FILES[$file_field]["name"], PATHINFO_EXTENSION);
            ...
            if(!$this->upload->do_upload($file_field)){
                ...
                $this->setFlashAlert("error", "Error uploading file: $error");
                redirect(base_url("sliders/new"));
            }
            ...
            if($insert){
                $this->setFlashAlert("success", "Record added successfully");
            } else {
                $this->setFlashAlert("error", "Record could not be added");
            }
    ...
    private function setFlashAlert($type, $message){
        $alert = array(
            "title" => $type == "success" ? $this->lang->line('operation-is-succesfull-message') : $this->lang->line('operation-is-unsuccesfull-message'),
            "text" => $message,
            "type"  => $type
        );
